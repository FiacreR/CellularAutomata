const s = ( p ) => {
  ...
  p.display31212 = function() {
    ...
    for (x = 0; x < p.simLength; x += 2) {
        for (y = 0; y < p.simHeight; y += 2) {
          if (p.alive[x][y]>0) {
            p.fill(((p.alive[x][y]-1)/(5*(numberOfStates-1))+0.01)%1,0.95,0.85);
            p.dodecagon((x)/2*p.spacer,(x/2+y)*p.spacer/1.7,p.spacer/p.scaling);
          }
        }
      }
    for (x = 1; x < p.simLength; x += 2) {
        for (y = 0; y < p.simHeight; y += 2) {
          if (p.alive[x][y]>0) {
            p.fill(((p.alive[x][y]-1)/(5*(numberOfStates-1))+0.01)%1,0.95,0.85);
            p.triangle((x-0.9)/2*p.spacer,(x/2+y-0.5)*p.spacer/1.7,p.spacer/6,p.TWO_PI)
          }
        }
      }
    for (x = 0; x < (p.width/p.spacer); x += 2) {
        for (y = 1; y < p.simHeight; y += 2) {
          if (p.alive[x][y]>0) {
            p.fill(((p.alive[x][y]-1)/(5*(numberOfStates-1))+0.01)%1,0.95,0.85);
            p.triangle((x-0.9)/2*p.spacer,(x/2+y)*p.spacer/1.7,p.spacer/6,p.TWO_PI/2)
          }
        }
      }
      p.pop()
  ...
  p.dodecagon = function(x, y, radius) {
    p.angle = p.TWO_PI / 12;
    p.beginShape();
    for (a = p.angle/2; a <= p.TWO_PI; a += p.angle) {
      p.sx = x + p.cos(a) * radius;
  ...
  p.apply31212 = function() {
    ...
    p.simLength = p.alive.length;
        for (x = 0; x < p.simLength; x += 2) {
        for (y = 0; y < p.simHeight; y += 2) {
          p.total[x][y] = [p.alive[(x-2+p.simLength)%p.simLength][y],
            p.alive[(x-2+p.simLength)%p.simLength][(y+1+p.simHeight)%p.simHeight],
            p.alive[(x-2+p.simLength)%p.simLength][(y+2+p.simHeight)%p.simHeight],
            p.alive[(x-1+p.simLength)%p.simLength][y],
            p.alive[(x-1+p.simLength)%p.simLength][(y+2+p.simHeight)%p.simHeight],
            p.alive[x][(y-2+p.simHeight)%p.simHeight],
            p.alive[x][(y-1+p.simHeight)%p.simHeight],
            ...
            p.alive[(x+1+p.simLength)%p.simLength][y],
            p.alive[(x+2+p.simLength)%p.simLength][(y-2+p.simHeight)%p.simHeight],
            p.alive[(x+2+p.simLength)%p.simLength][y]]
            .filter(elements => elements==1).length;
        }
      }
    for (x = 1; x < p.simLength; x += 2) {
    ...
    for (x = 0; x < p.simLength; x += 2) {
        for (y = 1; y < p.simHeight; y += 2) {
          p.total[x][y] = [p.alive[x][(y-1+p.simHeight)%p.simHeight],
            p.alive[x][(y+1+p.simHeight)%p.simHeight],
            p.alive[(x+2+p.simLength)%p.simLength][(y-1+p.simHeight)%p.simHeight]]
            .filter(elements => elements==1).length;
        }
